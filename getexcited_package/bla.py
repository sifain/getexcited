...
def bla():
    ...
    if dynq == 1: ## single trajectory
        ...
        if ctraj == 0:
            ...
            for ncoord in np.arange(ncoords):
                print >> output, '%0*.2f' % (len(str((tsmax))) + 2,dt*odata*cdata*ncoord), '%08.3f' % (sbla[ncoord])
        ...
        for NEXMD in NEXMDs:
            ...
            for dir in dirlist1:
                ...
                for ncoord in np.arange(ncoords):
                    ...
                    c = np.linalg.norm(np.subtract(vec3, vec2))
                    bla = (a + c)/2.0 - b
                    print >> output, '%s%04d' % (NEXMD,dir), '%0*.2f' % (len(str((tsmax))) + 2,dt*odata*cdata*ncoord), '%08.3f' % (bla)
                print '%s%04d' % (NEXMD,dir), '%0*.2f' % (len(str((tsmax))) + 2, (tsteps - 1)*dt)
                if tsteps == tsmax:
                    etraj += 1
                else:
